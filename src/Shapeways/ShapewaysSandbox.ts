export class ShapewaysSandbox {
  static GenPolyhedron() {
    ...
    for (let tile: Tile in tiling.Tiles) {
      for (let seg: Segment in tile.Boundary.Segments) {
        ...
        //     continue;
        if (
          SphericalCoords.CartesianToSpherical(v1).Y < Math.PI / 12 &&
        ...
        points = points
          .Select(() => {}, Sterographic.PlaneToSphereSafe(v))
          .ToArray()
        mesh.AddCurve(
          points,
          () => {},
          ShapewaysSandbox.SizeFunc(v, overallScale),
        )
      }
    }

    mesh.Mesh.Scale(overallScale)
    let outputFileName: string = `d:temp` + p + q + '.stl'
  ...
  static #SizeFunc(v: Vector3D, overallScale: number): number {
    ...
    //  angle 0 to 1
    let result: number = min + (max - min) * angle
    return result / overallScale
  }

  static GenCapWithHole() {
    let mesh: Shapeways = new Shapeways()
    let overallScale: number = 12.5
    //  2.5 cm = 1 in diameter
    //  Make hole 2 mm
    ...
    let div: number = 75
    let angleInc: number = (endAngle - startAngle) / div
    for (let i: number = 0; i < div; i++) {
      let angle1: number = startAngle + angleInc * i
      let angle2: number = startAngle + angleInc * (i + 1)
      ...
      let p1: Vector3D =
        s1 * (1 + ShapewaysSandbox.SizeFunc(s1, overallScale))
      let p2: Vector3D =
        s2 * (1 + ShapewaysSandbox.SizeFunc(s2, overallScale))
      let n1: Vector3D =
        s1 * (1 - ShapewaysSandbox.SizeFunc(s1, overallScale))
      let n2: Vector3D =
        s2 * (1 - ShapewaysSandbox.SizeFunc(s2, overallScale))
    ...
    mesh.Mesh.Scale(overallScale)
    let outputFileName: string = 'd:\tempcap_with_hole.stl'
    STL.SaveMeshToSTL(mesh.Mesh, outputFileName)
  }
  ...
  static GenCap() {
    ...
    let texCoords: Vector3D[] = TextureHelper.TextureCoords(
      poly,
      Geometry.Euclidean,
    )
    ...
    for (let i: number = 0; i < elements.Length / 3; i++) {
      ...
      v3.Normalize()
      let v1p: Vector3D =
        v1 * (1 + ShapewaysSandbox.SizeFunc(v1, overallScale))
      let v2p: Vector3D =
        v2 * (1 + ShapewaysSandbox.SizeFunc(v2, overallScale))
      let v3p: Vector3D =
        v3 * (1 + ShapewaysSandbox.SizeFunc(v3, overallScale))
      let v1n: Vector3D =
        v1 * (1 - ShapewaysSandbox.SizeFunc(v1, overallScale))
      let v2n: Vector3D =
        v2 * (1 - ShapewaysSandbox.SizeFunc(v2, overallScale))
    ...
    mesh.Mesh.Scale(overallScale)
    let outputFileName: string = 'd:\tempcap.stl'
    STL.SaveMeshToSTL(mesh.Mesh, outputFileName)
  }
}
