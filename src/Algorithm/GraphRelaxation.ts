...
export class Graph {
  ...
  m_connections: Record<number, Array<number>>

  constructor() {
    this.Nodes = new Array<GraphNode>()
    this.Edges = new Array<GraphEdge>()
    // this.m_connections = new Record<number, Array<number>>()
    this.m_connections = {}
  }
  ...
  AddEdge(e: GraphEdge) {
    ...
    if (m_connections.TryGetValue(e.V1, /* out */ vals)) {
      ...
      m_connections[e.V1] = new Array<number>([e.V2])
    }
  ...
  Normalize() {
    ...
    for (let node: GraphNode of this.Nodes) {
      node.Position /= mag
...
export class GraphNode {
  ...
  constructor(pos: VectorND, vel: VectorND) {
    this.Position = pos
    this.Velocity = vel
    this.Lock = new VectorND(pos.Dimension)
    //  No locking.
  }
...
export class GraphRelaxation {
  ...
  Relax(steps: number) {
    ...
    for (let i: number = 0; i < steps; i++) {
      ...
      let accelerations: Array<VectorND> = this.CalcAccelerations()
  ...
  #CalcAccelerations(): Array<VectorND> {
    let count: number = this.Graph.Nodes.Count
    let accelerations: Array<VectorND> = new Array(count)
