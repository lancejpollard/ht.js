export class VRML {
  ...
  static LoadIRP(path: string): VRMLInfo {
    try {
      ...
      let lines: Array<string> = File.ReadAllLines(path)
  ...
  static #LoadInternal(lines: Array<string>): VRMLInfo {
  ...
  static #LoadField(
    ...
    let split: Array<string> = VRML.SplitLine(line)
    if (split.Length != 4) {
  ...
  static #LoadPoints(
    ...
    while (true) {
      ...
      let split: Array<string> = VRML.SplitLine(cleaned)
      if (split.Length == 0) {
        continue
      }
  ...
  static #SplitLine(line: string): Array<string> {
    return line.Split(
      [' ', ',', '\t'],
  ...
  static #WritePoints(sw: StreamWriter, points: Array<Vector3D>) {
...
export class VRMLInfo {
  ...
  set Polygons(value: Array<Polygon>) {}
}
