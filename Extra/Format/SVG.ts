export class SVG {
  static WritePolygons(file: string, polygons: List<Polygon>) {
    ...
    let xDocument: XDocument = new XDocument(
      new XElement(
        'svg',
        ...
        new XElement(
          ...
          new XAttribute(
            'style',
            'fill:none;stroke:#0000FF;stroke-width:0.00005in',
          ),
          new XElement(
            'circle',
            new XAttribute('cx', '0'),
          ...
          polygons.Select(() => {}, SVG.XPolygon(poly)),
        ),
      ),
    )
    xDocument.Save(file)
  ...
  static WriteSegments(file: string, segs: List<Segment>) {
    let xDocument: XDocument = new XDocument(
      new XElement(
        'svg',
        new XAttribute('width', '5in'),
  ...
  static #XPolygon(poly: Polygon): XElement {
                ...
                (SVG.FormatDouble(seg.Radius, scale) +
                  (' 0 ' +
                    (SVG.FormatBool(largeArc) +
                      (SVG.FormatBool(sweepDirection) +
                        SVG.FormatPoint(seg.P2, scale))))))))
      } else {
  ...
  static #XSegment(seg: Segment): XElement {
    let poly: Polygon = new Polygon()
    poly.Segments.Add(seg)
    return SVG.XPolygon(poly)
  }

  static #FormatBool(value: boolean): string {
    return value ? '1 ' : '0 '
  }

  static #FormatPoint(p: Vector3D, scale: number): string {
    return (
      SVG.FormatDouble(p.X, scale) +
      (',' + (SVG.FormatDouble(p.Y, scale) + ' '))
    )
  }

  static #FormatDouble(d: number, scale: number): string {
