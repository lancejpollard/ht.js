...
module R3.Geometry {
    ...
    export class Shapeways {
        ...
        public set Div(value: number)  {
        }

        ///  <summary>
        ///  Add a sphere to a mesh.
        ///  </summary>
        public AddSphere(center: Vector3D, radius: number) {
            ...
            for (let i: number = 0; (i <= n2); i++) {
                let theta: number = (((Math.PI / 2)
                            * -1)
                            + (Math.PI
                            / (n2 * i)));
                let offset: Vector3D = new Vector3D(0, 0, (Math.Sin(theta) * radius));
            ...
            this.AddCap(axisPoints[(axisPoints.Count - 1)], disks[(axisPoints.Count - 2)], /* reverse:*/ true);
        }

        ///  <summary>
        ///  Add a curve with a constant radius size.
        ///  </summary>
        public AddCurve(points: Vector3D[], size: number) {
            let sizeFunc: System.Func<Vector3D, number>;
            size;
        ...
        ///  </summary>
        public AddCurve(points: Vector3D[], sizeFunc: System.Func<Vector3D, number>) {
            if ((points.Length < 2)) {
                throw new Error('Argument Error')("AddCurve requires at least two input points.");
            }

            let disks: List<Vector3D[]> = this.CalcDisks(points, sizeFunc);
        ...
        ///  XXX - Deprecated: sizeFunc return a position and a radius.
        ///  </summary>
        public AddCurve(points: Vector3D[], sizeFunc: System.Func<Vector3D, number>, start: Vector3D, end: Vector3D) {
            if ((points.Length < 2)) {
                throw new Error('Argument Error')("AddCurve requires at least two input points.");
            }
        ...
        public AddClosedCurve(points: Vector3D[], radii: number[]) {
            if ((points.Length < 3)) {
                throw new Error('Argument Error')("AddCurve requires at least three input points.");
            }
            ...
            let disks: List<Vector3D[]> = this.CalcDisks(points, radii);
        ...
        public AddCurve(points: Vector3D[], radii: number[]) {
            if ((points.Length < 2)) {
                throw new Error('Argument Error')("AddCurve requires at least two input points.");
            }
        ...
        private AddCurve(disks: List<Vector3D[]>, points: Vector3D[], start: Vector3D, end: Vector3D) {
            //  Cap1
        ...
        public AddArc(center: Vector3D, arcRadius: number, v1: Vector3D, normal: Vector3D, angleTot: number, numPoints: number, sizeFunc: System.Func<Vector3D, number>) {
            if ((numPoints < 2)) {
                numPoints = 2;
            }

            let points: Vector3D[] = Shapeways.CalcArcPoints(center, arcRadius, v1, normal, angleTot, numPoints);
            //  Calculate all the disks.
            //  XXX - duplicated code with CalcDisks, but we need to calc our own perpendicular here.
            let disks: List<Vector3D[]> = new List<Vector3D[]>();
            for (let i: number = 0; (i < points.Length); i++) {
                ...
                axis.RotateAboutAxis(normal, ((Math.PI / 2)
                                * -1));
                perp = (perp * radius);
                disks.Add(Shapeways.Disk(p2, axis, perp, this.Div, /* reverse:*/ false));
            }
        ...
        ///  </summary>
        public static CalcArcPoints(center: Vector3D, radius: number, v1: Vector3D, normal: Vector3D, angleTot: number, numPoints: number): Vector3D[] {
        ...
        ///  </summary>
        public AddCornucopia(outerPoints: Vector3D[], outerSizeFunc: System.Func<Vector3D, number>, innerPoints: Vector3D[], innerSizeFunc: System.Func<Vector3D, number>) {
            if ((outerPoints.Length < 2)) {
                throw new Error('Argument Error')("AddCornucopia requires at least two input points.");
            }

            let disksOuter: List<Vector3D[]> = this.CalcDisks(outerPoints, outerSizeFunc);
            let disksInner: List<Vector3D[]> = this.CalcDisks(innerPoints, innerSizeFunc, /* reverse:*/ true);
            //  Cap1
            this.AddCap(outerPoints[0], disksOuter[0]);
            //  Interior points.
            for (let i: number = 0; (i
                        < (disksOuter.Count - 1)); i++) {
            ...
            //  Interior points.
            for (let i: number = 0; (i
                        < (disksInner.Count - 1)); i++) {
                this.AddSegment(disksInner[i], disksInner[(i + 1)]);
            }

            //  Interior cap.
            this.AddCap(innerPoints[(innerPoints.Length - 1)], disksInner[(innerPoints.Length - 1)], /* reverse:*/ true);
        }

        ///  <summary>
        ///  Helper to calculate a set of disks perpendicular to a polyline.
        ///  </summary>
        private CalcDisks(points: Vector3D[], sizeFunc: System.Func<Vector3D, number>, reverse: boolean = false): List<Vector3D[]> {
            let disks: List<Vector3D[]> = new List<Vector3D[]>();
            for (let i: number = 0; (i < points.Length); i++) {
                let p1: Vector3D = points[0];
                // TODO: Warning!!!, inline IF is not supported ?
                ...
                disks.Add(Shapeways.Disk(p2, axis, perpendicular, this.Div, reverse));
            }

            return disks;
        }

        private CalcDisks(points: Vector3D[], radii: number[], reverse: boolean = false): List<Vector3D[]> {
            let disks: List<Vector3D[]> = new List<Vector3D[]>();
            for (let i: number = 0; (i < points.Length); i++) {
                let p1: Vector3D = points[0];
                // TODO: Warning!!!, inline IF is not supported ?
        ...
        ///  </summary>
        private AddCap(center: Vector3D, diskPoints: Vector3D[], reverse: boolean = false) {
            for (let i: number = 0; (i < diskPoints.Length); i++) {
                ...
                (i + 1);
                if (reverse) {
                    this.Mesh.Triangles.Add(new Mesh.Triangle(center, diskPoints[idx2], diskPoints[idx1]));
                }
                else {
                    this.Mesh.Triangles.Add(new Mesh.Triangle(center, diskPoints[idx1], diskPoints[idx2]));
                }
        ...
        ///  Adds an annulus.
        ///  </summary>
        private AddRing(disk1Points: Vector3D[], disk2Points: Vector3D[]) {
        ...
        ///  </summary>
        public AddSegment(d1: Vector3D[], d2: Vector3D[]) {
        ...
        ///  </summary>
        private static CurveAxis(p1: Vector3D, p2: Vector3D, p3: Vector3D): Vector3D {
        ...
        ///  </summary>
        private static CurvePerp(p1: Vector3D, p2: Vector3D, p3: Vector3D): Vector3D {
        ...
        ///  </summary>
        public static Disk(p: Vector3D, axis: Vector3D, perpendicular: Vector3D, divisions: number, reverse: boolean = false): Vector3D[] {
            let points: List<Vector3D> = new List<Vector3D>();
