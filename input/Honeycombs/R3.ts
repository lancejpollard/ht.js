...
module R3.Geometry {
    ...
    export class Euclidean {
        ...
        public static GenEuclidean() {
            let mesh: Shapeways = new Shapeways();
            let completed: HashSet<H3.Cell.Edge> = new HashSet<H3.Cell.Edge>(new H3.Cell.EdgeEqualityComparer());
            let count: number = 2;
            for (let i: number = (count * -1); (i < count); i++) {
                for (let j: number = (count * -1); (j < count); j++) {
                    for (let k: number = (count * -1); (k < count); k++) {
                        ...
                        Euclidean.AddEuclideanEdge(mesh, completed, p, new Vector3D((io + 1), jo, ko));
                        Euclidean.AddEuclideanEdge(mesh, completed, p, new Vector3D((io - 1), jo, ko));
                        Euclidean.AddEuclideanEdge(mesh, completed, p, new Vector3D(io, (jo + 1), ko));
                        Euclidean.AddEuclideanEdge(mesh, completed, p, new Vector3D(io, (jo - 1), ko));
                        Euclidean.AddEuclideanEdge(mesh, completed, p, new Vector3D(io, jo, (ko + 1)));
                        Euclidean.AddEuclideanEdge(mesh, completed, p, new Vector3D(io, jo, (ko - 1)));
                    }
        ...
        private static AddEuclideanEdge(mesh: Shapeways, completed: HashSet<H3.Cell.Edge>, start: Vector3D, end: Vector3D) {
            let cutoff: number = 1.75;
            if (((Math.abs(start.X) > cutoff)
        ...
        private static AddEuclideanEdgeToMesh(mesh: Shapeways, completed: HashSet<H3.Cell.Edge>, start: Vector3D, end: Vector3D) {
            ...
            tempMesh.AddCurve(seg.Subdivide(div), ., 5);
            // Transform( tempMesh.Mesh );
            mesh.Mesh.Triangles.AddRange(tempMesh.Mesh.Triangles);
            completed.Add(edge);
        }

        private static Transform(mesh: Mesh) {
            let sphere: Sphere = new Sphere();
            sphere.Radius = 0.1;
            for (let i: number = 0; (i < mesh.Triangles.Count); i++) {
                mesh.Triangles[i] = new Mesh.Triangle(sphere.ReflectPoint(mesh.Triangles[i].a), sphere.ReflectPoint(mesh.Triangles[i].b), sphere.ReflectPoint(mesh.Triangles[i].c));
            }
