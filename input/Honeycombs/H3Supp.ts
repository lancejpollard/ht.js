...
module R3.Geometry {
    ...
    export class H3Supp {
        ...
        ///  </summary>
        public static GenerateExotic(honeycomb: EHoneycomb, settings: H3.Settings) {
            settings.AngularThickness = 0.17;
            let tiling: Tiling;
            let baseTile: Tile;
            H3Supp.GetAssociatedTiling(honeycomb, /* out */tiling, /* out */baseTile);
            ...
            if (m_params.RemoveDangling) {
                let edgeDict: Record<H3.Cell.Edge, number> = edges.ToRecord(() => {  }, e, () => {  }, 1);
                H3.RemoveDanglingEdgesRecursive(edgeDict);
                edges = edgeDict.Keys.ToList();
            }

            let outputFileName: string = (H3.m_baseDir + Honeycomb.String(honeycomb, false));
            ...
            if ((m_params.Output == H3.Output.STL)) {
                ...
                outputFileName += ".pov";
                PovRay.WriteH3Edges(new PovRay.Parameters(), edges.ToArray(), outputFileName);
            }
        ...
        class Params {
            ...
            public UhsCutoff: number;

            public BallCutoff: number;

            public RemoveDangling: boolean;

            public Output: H3.Output = H3.Output.POVRay;
        ...
        private static m_params: Params = new Params();

        public static IsExotic(honeycomb: EHoneycomb): boolean {
            return ((honeycomb == EHoneycomb.H444)
                        || ((honeycomb == EHoneycomb.H636)
                        || (honeycomb == EHoneycomb.H363)));
        }

        private static GetPQ(honeycomb: EHoneycomb, /* out */p: number, /* out */q: number) {
            let r: number;
            Honeycomb.PQR(honeycomb, /* out */p, /* out */q, /* out */r);
        }

        private static GetAssociatedTiling(honeycomb: EHoneycomb, /* out */tiling: Tiling, /* out */baseTile: Tile) {
            let q: number;
            let p: number;
            H3Supp.GetPQ(honeycomb, /* out */p, /* out */q);
            let tilingConfig: TilingConfig = new TilingConfig(p, q, /* maxTiles:*/ m_params.MaxTiles);
            tiling = new Tiling();
        ...
        ///  </summary>
        private static DoOneStep(regionEdges: List<H3.Cell.Edge>, tiling: Tiling, region: Circle): List<H3.Cell.Edge> {
            let newEdges: HashSet<H3.Cell.Edge> = new HashSet<H3.Cell.Edge>(new H3.Cell.EdgeEqualityComparer());
        ...
        private static EdgeOkUHS(edge: H3.Cell.Edge, region: Circle): boolean {
            if ((Tolerance.GreaterThan(edge.Start.Abs(), region.Radius) || Tolerance.GreaterThan(edge.End.Abs(), region.Radius))) {
                return false;
            }
        ...
        private static EdgeOkBall(edge: H3.Cell.Edge): boolean {
            return H3Supp.EdgeOk(edge, m_params.BallCutoff);
        }

        private static EdgeOk(edge: H3.Cell.Edge, cutoff: number): boolean {
            return (edge.Start.Dist(edge.End) > cutoff);
        }

        private static CopyAndProject(regionEdges: List<H3.Cell.Edge>, tiling: Tiling, scale: number, offset: Vector3D): List<H3.Cell.Edge> {
            let newEdges: HashSet<H3.Cell.Edge> = new HashSet<H3.Cell.Edge>(new H3.Cell.EdgeEqualityComparer());
        ...
        private static FaceCenteredScale(vertexCircle: Circle): number {
            //  The radius is the height of the face in UHS.
            //  We need to scale this to be at (0,0,1)
            return (1 / vertexCircle.Radius);
        }

        private static EdgeCenteredScale(baseTile: Tile): number {
