...
module R3.Geometry {
    ...
    class Lamp {
        ...
        private static m_inventory: Inventory = new Inventory();

        ///  <summary>
        ///  Function I was using for lamp project, probably a bit out of date.
        ///  </summary>
        public static AddToMeshLamp(mesh: Shapeways, v1: Vector3D, v2: Vector3D) {
            ...
            return (outerRadStart
                        + ((outerRadEnd - outerRadStart)
                        * (len / length1)));
            ...
            let outerPoints: Vector3D[] = Shapeways.CalcArcPoints(center, radius, v1, normal, (length1 / radius));
            let innerPoints: Vector3D[] = Shapeways.CalcArcPoints(center, radius, outerPoints[(outerPoints.Length - 1)], (normal * -1), (length2 / radius));
        ...
        private static Scale(rad: number): number {
    ...
    class Rod {
        ...
        public static Create(radius: number, angle: number): Rod {
            let r: Rod = new Rod();
            r.Radius = radius;
        ...
        public static Operator(r1: Rod, r2: Rod): boolean {
        ...
        public /* override */ GetHashCode(): number {
            ...
            return (Math.Round(this.Radius, decimals).GetHashCode() | Math.Round(this.Length, decimals).GetHashCode());
            // The operator should be an XOR ^ instead of an OR, but not available in CodeDOM
        }

        public Compare(other: Rod, threshold: number): boolean {
            return (Tolerance.Equal(this.Radius, other.Radius, threshold) && Tolerance.Equal(this.Length, other.Length, threshold));
        }
    }

    class Inventory {

        public Rods: Record<Rod, number> = new Record<Rod, number>();
