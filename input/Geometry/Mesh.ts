...
module R3.Geometry {
    ...
    export class Mesh {
        ...
        export struct Triangle {
            ...
            public constructor (_a: Vector3D, _b: Vector3D, _c: Vector3D) {
                a = _a;
                b = _b;
                c = _c;
        ...
        public set Triangles(value: List<Triangle>)  {
        }

        public Append(m: Mesh) {
            this.Triangles.AddRange(m.Triangles);
        }

        public BuildIndexes(/* out */verts: Vector3D[], /* out */normals: Vector3D[], /* out */faces: List<number[]>) {
            let vertMap: Record<Vector3D, number> = new Record<Vector3D, number>();
            let triMap: Record<Vector3D, List<Triangle>> = new Record<Vector3D, List<Triangle>>();
        ...
        public Scale(scale: number) {
            for (let i: number = 0; (i < this.Triangles.Count); i++) {
                this.Triangles[i] = new Mesh.Triangle((this.Triangles[i].a * scale), (this.Triangles[i].b * scale), (this.Triangles[i].c * scale));
            }
        ...
        ///  </summary>
        public Transform(transform: System.Func<Vector3D, Vector3D>) {
            for (let i: number = 0; (i < this.Triangles.Count); i++) {
                this.Triangles[i] = new Mesh.Triangle(transform(this.Triangles[i].a), transform(this.Triangles[i].b), transform(this.Triangles[i].c));
            }
        ...
        ///  </summary>
        public AddBand(d1: Vector3D[], d2: Vector3D[], close: boolean = true) {
            if ((d1.Length != d2.Length)) {
                throw new Error('Argument Error')("Edges must have the same length.");
            }
            ...
            for (let i: number = 0; (i < end); i++) {
                ...
                (i
                            == (d1.Length - 1));
                (i + 1);
                this.Triangles.Add(new Mesh.Triangle(d1[idx1], d2[idx1], d1[idx2]));
                this.Triangles.Add(new Mesh.Triangle(d1[idx2], d2[idx1], d2[idx2]));
            }
        ...
        ///  </summary>
        public static MakeEdgeMesh(p: number, q: number): Mesh {
            let mesh: Mesh = new Mesh();
            let maxTiles: number = 400;
            let tiling: Tiling = new Tiling();
            let config: TilingConfig = new TilingConfig(p, q, /* maxTiles:*/ maxTiles);
            config.Shrink = 0.6;
            tiling.GenerateInternal(config);
            let boundaryConfig: TilingConfig = new TilingConfig(14, 7, /* maxTiles:*/ 1);
        ...
        private static m_divisions: number = 75;

        private static Shrink(p: Vector3D, centroid: Vector3D): Vector3D {
        ...
        private static AddSymmetryTriangles(mesh: Mesh, tiling: Tiling, boundary: Polygon) {
            ...
            for (let seg: Segment in template.Segments) {
                ...
                let coords: Vector3D[] = TextureHelper.TextureCoords(poly, Geometry.Hyperbolic);
                let elements: number[] = TextureHelper.TextureElements(3, /* LOD:*/ 3);
            ...
            for (let tile: Tile in tiling.Tiles) {
                ...
                for (let tri: Triangle in templateTris) {
                    let transformed: Triangle = new Triangle(m.Apply(tri.a), m.Apply(tri.b), m.Apply(tri.c));
        ...
        private static MeshEdges(mesh: Mesh, tile: Tile, completed: HashSet<Vector3D>, boundary: Polygon) {
            for (let i: number = 0; (i < tile.Boundary.Segments.Count); i++) {
                ...
                for (let incident: Tile in tile.EdgeIncidences) {
                    for (let j: number = 0; (j < incident.Boundary.Segments.Count); j++) {
                ...
                if (foundIncident) {
                    Mesh.CheckAndAdd(mesh, new Mesh.Triangle(boundarySeg.P1, d1.P1, d2.P1), boundary);
                    Mesh.CheckAndAdd(mesh, new Mesh.Triangle(boundarySeg.P2, d2.P2, d1.P2), boundary);
                }
                ...
                for (let j: number = 0; (j < num); j++) {
                    Mesh.CheckAndAdd(mesh, new Mesh.Triangle(list1[j], list1[(j + 1)], list2[(j + 1)]), boundary);
                    Mesh.CheckAndAdd(mesh, new Mesh.Triangle(list2[j], list1[j], list2[(j + 1)]), boundary);
                }
        ...
        private static Check(tri: Triangle, boundary: Polygon): boolean {
            if (((boundary == null)
                        || (boundary.IsPointInside(tri.a)
                        && (boundary.IsPointInside(tri.b) && boundary.IsPointInside(tri.c))))) {
                return true;
            }
        ...
        private static CheckAndAdd(mesh: Mesh, tri: Triangle, boundary: Polygon) {
