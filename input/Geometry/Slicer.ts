...
module R3.Geometry {
    ...
    export class Slicer {
        ...
        public static SlicePolygon(p: Polygon, c: CircleNE, g: Geometry, thickness: number, /* out */output: List<Polygon>) {
            ...
            let pointOnCircle: Vector3D = c.P1;
        ...
        ///  </summary>
        ///  <remarks>The input polygon might get reversed</remarks>
        public static SlicePolygon(p: Polygon, c: Circle, /* out */output: List<Polygon>): boolean {
        ...
        ///  </summary>
        public static Clip(/* ref */tiles: List<Tile>, c: Circle, keepInside: boolean) {
            ...
            for (let t: Tile in tiles) {
                ...
                for (let p: Polygon in sliced) {
                    let insideCircle: boolean = (((p.CentroidApprox - c.Center)).Abs() < c.Radius);
                    if (((keepInside && insideCircle)
                                || (!keepInside
                                && !insideCircle))) {
        ...
        private static SlicePolygonInternal(p: Polygon, c: Circle, /* out */output: List<Polygon>): boolean {
            //  Our approach:
            ...
            for (let i: number = 0; (i < p.NumSides); i++) {
                ...
                let intersections: Vector3D[] = c.GetIntersectionPoints(s);
                ...
                switch (intersections.Length) {
                    ...
                    case 1:
                        ...
                        diced.Segments.Add(Slicer.SplitHelper(s, intersections[0], diced, iPoints));
                        break;
                        break;
                    case 2:
                        ...
                        let secondToSplit: Segment = Slicer.SplitHelper(s, i1, diced, iPoints);
                        let segmentToAdd: Segment = Slicer.SplitHelper(secondToSplit, i2, diced, iPoints);
            ...
            if ((iPoints.Count > 2)) {
                ...
                let dummy: number = 0;
                let testArc: Segment = Slicer.SmallerSplicedArc(c, iPoints, /* ref */dummy, true, /* ref */dummy);
        ...
        ///  </summary>
        private static SplitHelper(segmentToSplit: Segment, iLocation: Vector3D, diced: Polygon, iPoints: List<IntersectionPoint>): Segment {
        ...
        ///  </summary>
        private static WalkPolygon(parent: Polygon, walking: Polygon, c: Circle, pair: number, iPoints: List<IntersectionPoint>, increment: boolean): Polygon {
            let newPoly: Polygon = new Polygon();
            let iPoint2: IntersectionPoint;
            let iPoint1: IntersectionPoint;
            Slicer.GetPairPoints(iPoints, pair, increment, /* out */iPoint1, /* out */iPoint2);
            let startLocation: Vector3D = iPoint1.Location;
            let iSeg: number = 0;
            let current: Segment = Slicer.SplicedArc(parent, c, iPoints, /* ref */pair, increment, /* ref */iSeg);
            ...
            while (true) {
                ...
                if (iPoints.Select(() => {  }, p.Location).Contains(segEnd)) {
                    current = Slicer.SplicedArc(parent, c, iPoints, /* ref */pair, increment, /* ref */iSeg);
                    newPoly.Segments.Add(current.Clone());
                    if (current.P2.Compare(startLocation)) {
                        break;
                    }
        ...
        ///  </summary>
        private static GetPairPoints(iPoints: List<IntersectionPoint>, pair: number, increment: boolean, /* out */iPoint1: IntersectionPoint, /* out */iPoint2: IntersectionPoint) {
            let idx1: number = (pair * 2);
            let idx2: number = ((pair * 2)
                        + 1);
            if (!increment) {
        ...
        ///  </summary>
        private static SmallerSplicedArc(c: Circle, iPoints: List<IntersectionPoint>, /* ref */pair: number, increment: boolean, /* ref */nextSegIndex: number): Segment {
            let iPoint2: IntersectionPoint;
            let iPoint1: IntersectionPoint;
            Slicer.GetPairPoints(iPoints, pair, increment, /* out */iPoint1, /* out */iPoint2);
            ...
            let newSeg: Segment = Segment.Arc(p1, p2, c.Center, /* clockwise:*/ true);
        ...
        ///  </summary>
        private static SplicedArc(parent: Polygon, c: Circle, iPoints: List<IntersectionPoint>, /* ref */pair: number, increment: boolean, /* ref */nextSegIndex: number): Segment {
            let spliced: Segment = Slicer.SmallerSplicedArc(c, iPoints, /* ref */pair, increment, /* ref */nextSegIndex);
        ...
        private static GetNextSegmentIndex(walking: Polygon, idx: number): number {
