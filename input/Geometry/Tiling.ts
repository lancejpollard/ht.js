...
module R3.Geometry {
    ...
    ///  </summary>
    export struct TilingConfig {

        public constructor (p: number, q: number, maxTiles: number) {
        ...
        private SetupConfig(p: number, q: number, maxTiles: number) {
            P = p;
            Q = q;
            m.Unity();
            MaxTiles = maxTiles;
        ...
        public set MaxTiles(value: number)  {
        }
        ...
        public static VertexCenteredMobius(p: number, q: number): Mobius {
            let angle: number = (Math.PI / q);
            if (Utils.Even(q)) {
                angle = (angle * 2);
            }

            let offset: Vector3D = new Vector3D(((1 * Geometry2D.GetNormalizedCircumRadius(p, q))
                            * -1), 0, 0);
            offset.RotateXY(angle);
            let m: Mobius = new Mobius();
            this.m.Isometry(Geometry2D.GetGeometry(p, q), angle, offset.ToComplex());
            return this.m;
        }
    ...
    export class Tiling {
        ...
        ///  </summary>
        public get TilingConfig(): TilingConfig {
        }
        public set TilingConfig(value: TilingConfig)  {
        }
        ...
        ///  </summary>
        public get TilePositions(): Record<Vector3D, Tile> {
        }
        public set TilePositions(value: Record<Vector3D, Tile>)  {
        }
        ...
        public static MakeDualTilings(/* out */tiling1: Tiling, /* out */tiling2: Tiling, p: number, q: number) {
            tiling1 = new Tiling();
            tiling2 = new Tiling();
        ...
        ///  </summary>
        public Generate(config: TilingConfig) {
            this.GenerateInternal(config);
        }

        public GenerateInternal(config: TilingConfig, projection: Polytope.Projection = Polytope.Projection.FaceCentered) {
            ...
            tiles.Add(tile);
            let completed: Record<Vector3D, boolean> = new Record<Vector3D, boolean>();
        ...
        ///  </summary>
        private static FillOutIsometries(home: Tile, tiles: List<Tile>, g: Geometry) {
        ...
        ///  </summary>
        private FillOutIncidences() {
            let Edges: Record<Vector3D, List<Tile>> = new Record<Vector3D, List<Tile>>();
            let Vertices: Record<Vector3D, List<Tile>> = new Record<Vector3D, List<Tile>>();
            ...
            for (let t: Tile in this.m_tiles) {
                ...
                t.EdgeIncidences = t.EdgeIncidences.Distinct().Except([
                            t]).ToList();
                t.VertexIndicences = t.VertexIndicences.Distinct().Except([
                            t]).ToList();
        ...
        public static CreateBaseTile(config: TilingConfig): Tile {
        ...
        ///  </summary>
        public Clip(c: Circle, interior: boolean = true) {
        ...
        ///  </summary>
        private TransformAndAdd(tile: Tile): boolean {
        ...
        ///  </summary>
        public NewTileAfterReflect(t: Tile, s: Segment, completed: Record<Vector3D, boolean>): boolean {
            let newVertexCircle: CircleNE = t.VertexCircle.Clone();
            newVertexCircle.Reflect(s);
            let testCenter: Vector3D = this.TilingConfig.M.Apply(newVertexCircle.CenterNE);
            return !completed.ContainsKey(testCenter);
        }

        private ReflectRecursive(tiles: List<Tile>, completed: Record<Vector3D, boolean>) {
            ...
            for (let tile: Tile in tiles) {
                ...
                for (let s: number = 0; (s < tile.Boundary.NumSides); s++) {
                    ...
                    if (this.TransformAndAdd(newBase)) {
                        console.assert(!completed.ContainsKey(newBase.Boundary.Center));
        ...
        ///  </summary>
        public get Tiles(): IEnumerable<Tile> {
            return this.m_tiles;
        }
        ...
        ///  </summary>
        public get Polygons(): IEnumerable<Polygon> {
        ...
        ///  </summary>
        public get Circles(): IEnumerable<CircleNE> {
