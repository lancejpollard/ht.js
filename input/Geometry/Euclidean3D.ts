...
module R3.Geometry {
    ...
    export class Euclidean3D {
        ...
        public static DistancePointLine(n1: Vector3D, p1: Vector3D, point: Vector3D): number {
        ...
        public static DistancePointPlane(normalVector: Vector3D, planePoint: Vector3D, point: Vector3D): number {
        ...
        public static ProjectOntoLine(nl: Vector3D, pl: Vector3D, point: Vector3D): Vector3D {
        ...
        public static ProjectOntoPlane(normalVector: Vector3D, planePoint: Vector3D, point: Vector3D): Vector3D {
            if (!normalVector.Normalize()) {
                throw new Error('Argument Error')("Invalid normal vector.");
            }

            let dist: number = Euclidean3D.DistancePointPlane(normalVector, planePoint, point);
            normalVector = (normalVector * dist);
            return (point - normalVector);
        }

        public static DistanceLineLine(n1: Vector3D, p1: Vector3D, n2: Vector3D, p2: Vector3D): number {
        ...
        ///  </summary>
        public static PointOnSegment(s1: Vector3D, s2: Vector3D, point: Vector3D): boolean {
        ...
        ///  </summary>
        public static DoSegmentsIntersect(a1: Vector3D, a2: Vector3D, b1: Vector3D, b2: Vector3D): boolean {
            //  (1)
            if ((Euclidean3D.PointOnSegment(a1, a2, b1)
                        || (Euclidean3D.PointOnSegment(a1, a2, b2)
                        || (Euclidean3D.PointOnSegment(b1, b2, a1) || Euclidean3D.PointOnSegment(b1, b2, a2))))) {
                return true;
            }
            ...
            if (Tolerance.GreaterThan(Euclidean3D.DistanceLineLine(ma, a1, mb, b1), 0)) {
                return false;
            }
        ...
        ///  to the points.
        ///  </summary>
        public static NormalFrom3Points(p1: Vector3D, p2: Vector3D, p3: Vector3D, transform: System.Func<Vector3D, Vector3D>): Vector3D {
        ...
        public static NormalFrom3Points(p1: Vector3D, p2: Vector3D, p3: Vector3D): Vector3D {
        ...
        public static TriangleAreaAfterTransform(/* ref */p1: Vector3D, /* ref */p2: Vector3D, /* ref */p3: Vector3D, transform: System.Func<Vector3D, Vector3D>): number {
            p1 = transform(p1);
            p2 = transform(p2);
            p3 = transform(p3);
            let v1: Vector3D = (p1 - p3);
        ...
        public static MaxTriangleEdgeLengthAfterTransform(/* ref */p1: Vector3D, /* ref */p2: Vector3D, /* ref */p3: Vector3D, transform: System.Func<Vector3D, Vector3D>): number {
            p1 = transform(p1);
            p2 = transform(p2);
            p3 = transform(p3);
        ...
        public static IntersectionPlaneLine(planeNormal: Vector3D, planePoint: Vector3D, nl: Vector3D, pl: Vector3D): Vector3D {
            let signedDistance: number = Euclidean3D.DistancePointPlane(planeNormal, planePoint, pl);
            ...
            return (pl
                        + (nl
                        * (signedDistance / Math.Cos(angle))));
            //  XXX - needs improvement.
        }

        public static IntersectionSphereLine(/* out */int1: Vector3D, /* out */int2: Vector3D, sphereCenter: Vector3D, sphereRadius: number, nl: Vector3D, pl: Vector3D): number {
            ...
            if ((distance < sphereRadius)) {
                ...
                let alpha: number = Utils.RadiansToDegrees(Math.Acos((distance / sphereRadius)));
                //  Rotation vector.
