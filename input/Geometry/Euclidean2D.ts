...
module R3.Geometry {
    ...
    export class Euclidean2D {
        ...
        ///  </summary>
        public static AngleToCounterClock(v1: Vector3D, v2: Vector3D): number {
        ...
        ///          So as coded, this function is really only intended to be used with 2D vector inputs.
        ///  </summary>
        public static AngleToClock(v1: Vector3D, v2: Vector3D): number {
            let result: number = Euclidean2D.AngleToCounterClock(v1, v2);
            return ((2 * Math.PI)
                        - result);
        }

        public static DistancePointLine(p: Vector3D, lineP1: Vector3D, lineP2: Vector3D): number {
            //  The line vector
        ...
        public static ProjectOntoLine(p: Vector3D, lineP1: Vector3D, lineP2: Vector3D): Vector3D {
            let v1: Vector3D = (lineP2 - lineP1);
            let lineMag: number = v1.Abs();
            if (Tolerance.Zero(lineMag)) {
                console.assert(false);
            ...
            return (lineP1
                        + (v1 * distanceAlongLine));
        }

        public static IntersectionLineLine(p1: Vector3D, p2: Vector3D, p3: Vector3D, p4: Vector3D, /* out */intersection: Vector3D): number {
            ...
            if (!Tolerance.Zero(Euclidean2D.DistancePointLine(intersection, p1, p2))) {
                // console.assert( false );
            }
        ...
        public static IntersectionCircleCircle(c1: Circle, c2: Circle, /* out */p1: Vector3D, /* out */p2: Vector3D): number {
            p1 = new Vector3D();
            p2 = new Vector3D();
            ...
            if ((Tolerance.GreaterThan(d, (r1 + r2)) || Tolerance.LessThan(d, Math.abs((r1 - r2))))) {
                return 0;
            }

            //  One intersection point.
            if ((Tolerance.Equal(d, (r1 + r2)) || Tolerance.Equal(d, Math.abs((r1 - r2))))) {
                p1 = (c1.Center
                            + (v * r1));
                return 1;
            }
            ...
            p2 = (v * r1);
            p1 = (v * r1);
            let temp: number = ((((r1 * r1)
                        - (r2 * r2))
                        + (d * d)) / (2 * d));
            let angle: number = Math.Acos((temp / r1));
            console.assert((!Tolerance.Zero(angle)
                            && !Tolerance.Equal(angle, Math.PI)));
            p1.RotateXY(angle);
            p2.RotateXY((angle * -1));
            p1 = (p1 + c1.Center);
            p2 = (p2 + c1.Center);
        ...
        public static IntersectionLineCircle(lineP1: Vector3D, lineP2: Vector3D, circle: Circle, /* out */p1: Vector3D, /* out */p2: Vector3D): number {
            p1 = new Vector3D();
            p2 = new Vector3D();
            //  Distance from the circle center to the closest point on the line.
            let d: number = Euclidean2D.DistancePointLine(circle.Center, lineP1, lineP2);
            //  No intersection points.
        ...
        ///  </summary>
        public static ReflectPointInLine(input: Vector3D, p1: Vector3D, p2: Vector3D): Vector3D {
        ...
        public static SameSideOfLine(lineP1: Vector3D, lineP2: Vector3D, test1: Vector3D, test2: Vector3D): boolean {
