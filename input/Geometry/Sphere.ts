...
module R3.Geometry {
    ...
    export class Circle3D {
        ...
        public set Normal(value: Vector3D)  {
        }

        public get Color(): System.Drawing.Color {
        }
        public set Color(value: System.Drawing.Color)  {
        }
        ...
        public constructor (t1: Vector3D, t2: Vector3D, t3: Vector3D) {
            let center: Vector3D;
            let radius: number;
        ...
        public static FromCenterAnd2Points(cen: Vector3D, p1: Vector3D, p2: Vector3D): Circle3D {
            let circle: Circle3D = new Circle3D();
            circle.Center = cen;
            circle.Radius = ((p1 - cen)).Abs();
            if (!Tolerance.Equal(circle.Radius, ((p2 - cen)).Abs())) {
                throw new Error('Argument Error')("Points are not on the same circle.");
            }
        ...
        public get PointOnCircle(): Vector3D {
        ...
        ///  </summary>
        public get RepresentativePoints(): Vector3D[] {
        ...
        ///  </summary>
        public Subdivide(n: number): Vector3D[] {
        ...
        private static From3Points(v1: Vector3D, v2: Vector3D, v3: Vector3D, /* out */center: Vector3D, /* out */radius: number) {
            ...
            let b2: number = (b * b);
            let c2: number = (c * c);
            let circumCenterBary: Vector3D = new Vector3D((a2
                            * (b2
                            + (c2 - a2))), (b2
                            * (c2
                            + (a2 - b2))), (c2
                            * (a2
                            + (b2 - c2))));
            (circumCenterBary.X
                        + (circumCenterBary.Y + circumCenterBary.Z));
            //  Normalize.
        ...
        ///  </summary>
        private static BaryToCartesian(t1: Vector3D, t2: Vector3D, t3: Vector3D, bary: Vector3D): Vector3D {
            return ((bary.X * t1)
                        + ((bary.Y * t2)
                        + (bary.Z * t3)));
        }
    ...
    export class Sphere {
        ...
        public constructor (center: Vector3D, radius: number) {
            this.Reset();
            Center = center;
            Radius = radius;
        }

        public static Plane(normal: Vector3D): Sphere {
            return Sphere.Plane(new Vector3D(), normal);
        }

        public static Plane(offset: Vector3D, normal: Vector3D): Sphere {
        ...
        ///  </summary>
        public get Offset(): Vector3D {
        }
        public set Offset(value: Vector3D)  {
        }
        ...
        private m_normal: Vector3D = Vector3D.DneVector();
        ...
        public set IsPlane(value: boolean)  {
        }
        ...
        public /* override */ Equals(obj: Object): boolean {
            ...
            return (Tolerance.Equal(this.Radius, s.Radius)
        ...
        public IsPointInside(test: Vector3D): boolean {
            ...
            if (this.IsPlane) {
                ...
                inside = Tolerance.LessThan(((test - this.Center)).Abs(), this.Radius);
        ...
        public IsPointOn(test: Vector3D): boolean {
            if (this.IsPlane) {
                let dist: number = Euclidean3D.DistancePointPlane(this.Normal, this.Offset, test);
        ...
        public IsPointInsideOrOn(test: Vector3D): boolean {
        ...
        public ReflectPoint(p: Vector3D): Vector3D {
            if (this.IsPlane) {
                ...
                let dist: number = Euclidean3D.DistancePointPlane(this.Normal, this.Offset, p);
                let offset: Vector3D = (this.Normal
                            * (dist * -2));
                return (p + offset);
            }
            else {
        ...
        public Reflect(sphere: Sphere) {
            ...
            if (this.IsPlane) {
                ...
                let projected: Vector3D = Euclidean3D.ProjectOntoPlane(this.Normal, this.Offset, sphere.Center);
            ...
            //  Is mirror a plane?
            if (sphere.IsPlane) {
                let projected: Vector3D = Euclidean3D.ProjectOntoPlane(sphere.Normal, sphere.Offset, this.Center);
        ...
        public static From4Points(s1: Vector3D, s2: Vector3D, s3: Vector3D, s4: Vector3D): Sphere {
            ...
            let o: Vector3D = ((temp_v1
                        + (temp_v2 + temp_v3)) * (1 / denominator));
        ...
        ///  </summary>
        public ProjectToSurface(p: Vector3D): Vector3D {
            if (this.IsPlane) {
        ...
        ///  </summary>
        public Intersection(s: Sphere): Circle3D {
        ...
        ///  </summary>
        public Intersection(c: Circle3D): System.Tuple<Vector3D, Vector3D> {
        ...
        public static RotateSphere(s: Sphere, axis: Vector3D, rotation: number) {
            if (s.IsPlane) {
        ...
        public static ScaleSphere(s: Sphere, factor: number) {
            if (s.IsPlane) {
        ...
        public static TranslateSphere(s: Sphere, t: Vector3D) {
            if (s.IsPlane) {
                let offsetAlongNormal: Vector3D = Euclidean3D.ProjectOntoLine(s.Normal, new Vector3D(), t);
