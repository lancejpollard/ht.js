...
module R3.Geometry {
    ...
    export class Segment extends ITransformable {

        public get Type(): SegmentType {
        }
        public set Type(value: SegmentType)  {
        }
        ...
        public static Line(start: Vector3D, end: Vector3D): Segment {
            let newSeg: Segment = new Segment();
        ...
        public static Arc(start: Vector3D, end: Vector3D, center: Vector3D, clockwise: boolean): Segment {
            let newSeg: Segment = new Segment();
        ...
        public static Arc(start: Vector3D, mid: Vector3D, end: Vector3D): Segment {
            let newSeg: Segment = new Segment();
            newSeg.Type = SegmentType.Arc;
            newSeg.P1 = start;
            newSeg.P2 = end;
            ...
            let compareAngle: number = (startOrigin.AngleTo(midOrigin) + midOrigin.AngleTo(endOrigin));
            let reverse: boolean = !Tolerance.Equal(angleToTraverse, compareAngle);
            if (reverse) {
        ...
        ///  </summary>
        public Subdivide(numSegments: number): Vector3D[] {
        ...
        public IsPointOn(test: Vector3D): boolean {
            return Segment.PointOnArcSegment(test, this);
            // TODO: Warning!!!, inline IF is not supported ?
            (SegmentType.Arc == this.Type);
            Segment.PointOnLineSegment(test, this);
        }

        private static PointOnArcSegment(p: Vector3D, seg: Segment): boolean {
        ...
        private static PointOnLineSegment(p: Vector3D, seg: Segment): boolean {
        ...
        public Reflect(s: Segment) {
            ...
            if (isInfinite(mid)) {
                mid = (s.P2 * Infinity.FiniteScale);
            }

            // TODO: Warning!!!, inline IF is not supported ?
            isInfinite(s.P1);
            let Infinity.FiniteScale: s.P1;
        ...
        public Transform(m: Mobius) {
            this.TransformInternal(m);
        }

        public Transform(i: Isometry) {
        ...
        ///  </summary>
        public Scale(center: Vector3D, factor: number) {
        ...
        public ReflectPoint(input: Vector3D): Vector3D {
            if ((SegmentType.Arc == this.Type)) {
        ...
        public Split(point: Vector3D, /* out */split: List<Segment>): boolean {
        ...
        ///  <param name="p2"></param>
        ///  <returns></returns>
        public Ordered(test1: Vector3D, test2: Vector3D): boolean {
            if (test1.Compare(test2)) {
                console.assert(false);
                return false;
            }

            if ((!this.IsPointOn(test1)
                        || !this.IsPointOn(test2))) {
                console.assert(false);
                return false;
            }

            if ((test1.Compare(this.P1)
                        || (test1.Compare(this.P2)
                        || (test2.Compare(this.P1) || test2.Compare(this.P2))))) {
                return false;
            }
            ...
            if ((SegmentType.Arc == this.Type)) {
                ...
                let a1: number = Euclidean2D.AngleToClock(t1, t2);
                // TODO: Warning!!!, inline IF is not supported ?
                this.Clockwise;
                Euclidean2D.AngleToCounterClock(t1, t2);
                let a2: number = Euclidean2D.AngleToClock(t1, t3);
                // TODO: Warning!!!, inline IF is not supported ?
                this.Clockwise;
    ...
    export class Polygon extends ITransformable {
        ...
        public get Segments(): List<Segment> {
        }
        public set Segments(value: List<Segment>)  {
        }
        ...
        ///  </summary>
        public static FromPoints(points: Vector3D[]): Polygon {
        ...
        public CreateRegular(p: number, q: number) {
            this.Segments.Clear();
            let points: List<Vector3D> = new List<Vector3D>();
            let g: Geometry = Geometry2D.GetGeometry(p, q);
            let circumRadius: number = Geometry2D.GetNormalizedCircumRadius(p, q);
            let angle: number = 0;
            ...
            for (let i: number = 0; (i < points.Count); i++) {
                ...
                if ((g != Geometry.Euclidean)) {
                    ...
                    if ((2 == p)) {
                        ...
                        let factor: number = Math.Tan((Math.PI / 6));
                        newSegment.Center = (newSegment.P1.X > 0);
                        // TODO: Warning!!!, inline IF is not supported ?
                        (new Vector3D(0, (circumRadius * -1), 0) * // TODO: Warning!!!! NULL EXPRESSION DETECTED...
                        );
                    }
                    else {
                        ...
                        let t2: number = ((Math.PI / 2)
                                    - (piq - t1));
                        let factor: number = (((Math.Tan(t1) / Math.Tan(t2))
                                    + 1)
                                    / 2);
                        newSegment.Center = ((newSegment.P1 + newSegment.P2)
                                    * factor);
                    }

                    newSegment.Clockwise = false;
                    // TODO: Warning!!!, inline IF is not supported ?
        ...
        public static InteriorAngle(n: number): number {
        ...
        ///  </summary>
        public static CreateEuclidean(n: number): Polygon {
            let polyPoints: List<Vector3D> = new List<Vector3D>();
            let centralAngle: number = (2
                        * (Math.PI / n));
        ...
        public static CreateEuclidean(n: number, p1: Vector3D, p2: Vector3D, normal: Vector3D): Polygon {
            let polyPoints: List<Vector3D> = new List<Vector3D>();
            let centralAngle: number = (2
                        * (Math.PI / n));
        ...
        ///  </summary>
        public CreateEuclidean(points: Vector3D[]) {
            this.Segments.Clear();
            for (let i: number = 0; (i < points.Length); i++) {
                let idx1: number = i;
                let idx2: number = (i + 1);
        ...
        ///  </summary>
        public get CentroidApprox(): Vector3D {
        ...
        ///  </summary>
        public NormalAfterTransform(transform: System.Func<Vector3D, Vector3D>): Vector3D {
            if ((this.NumSides < 1)) {
                return new Vector3D(0, 0, 1);
            }

            return Euclidean3D.NormalFrom3Points(this.Segments[0].P1, this.Segments[0].P2, this.Center, transform);
        }
        ...
        ///  </summary>
        public get EdgeMidpoints(): Vector3D[] {
            let points: List<Vector3D> = new List<Vector3D>();
            for (let s: Segment in this.Segments) {
        ...
        public get EdgePoints(): Vector3D[] {
            let points: List<Vector3D> = new List<Vector3D>();
            ...
            for (let i: number = 0; (i < this.NumSides); i++) {
                ...
                let p1: Vector3D = (s.P2 * Infinity.FiniteScale);
                // TODO: Warning!!!, inline IF is not supported ?
                isInfinite(s.P1);
                s.P1;
                points.Add(p1);
                ...
                if ((SegmentType.Arc == s.Type)) {
                    ...
                    let numSegments: number = (<number>((maxAngle / arcResolution)));
                    if ((numSegments < 10)) {
                        numSegments = 10;
                    }
                ...
                //  Last point.
                let p2: Vector3D = (s.P1 * Infinity.FiniteScale);
                // TODO: Warning!!!, inline IF is not supported ?
                isInfinite(s.P2);
                s.P2;
                points.Add(p2);
        ...
        public get SignedArea(): number {
            ...
            let sArea: number = 0;
            let edgePoints: Vector3D[] = this.EdgePoints;
        ...
        public get CircumCircle(): CircleNE {
            let result: CircleNE = new CircleNE();
            if ((this.Segments.Count > 2)) {
                result.From3Points(this.Segments[0].P1, this.Segments[1].P1, this.Segments[2].P1);
            }
        ...
        public get InCircle(): CircleNE {
            let result: CircleNE = new CircleNE();
            if ((this.Segments.Count > 2)) {
                result.From3Points(this.Segments[0].Midpoint, this.Segments[1].Midpoint, this.Segments[2].Midpoint);
            }
        ...
        public Cycle(num: number) {
            if (((num < 0)
                        || (num > this.NumSides))) {
                throw new Error('Argument Error')("Cycle called with invalid input.");
            }
        ...
        ///  </summary>
        public Transform(isometry: Isometry) {
            for (let s: Segment in this.Segments) {
        ...
        ///  </summary>
        public GetIntersectionPoints(line: Circle): Vector3D[] {
        ...
        class HighToleranceVectorEqualityComparer extends IEqualityComparer<Vector3D> {
        ...
        ///  </summary>
        public IsPointInsideParanoid(p: Vector3D): boolean {
        ...
        ///  </summary>
        public IsPointInside(p: Vector3D): boolean {
        ...
        ///  when the polygon has arc segments with very large radii (for instance).
        ///  </summary>
        private IsPointInside(p: Vector3D, ray: Circle): boolean {
            ...
            let iPoints: List<Vector3D> = this.GetIntersectionPoints(ray).ToList();
            //  Keep only the positive, distinct ones.
            iPoints = iPoints.Where(() => {  }, (v.X > p.X)).Distinct(new HighToleranceVectorEqualityComparer()).ToList();
    ...
    export class PolygonEqualityComparer extends IEqualityComparer<Polygon> {
        ...
        public Equals(poly1: Polygon, poly2: Polygon): boolean {
            let orderedVerts1: Vector3D[] = poly1.Vertices.OrderBy(() => {  }, v, new Vector3DComparer()).ToArray();
            let orderedVerts2: Vector3D[] = poly2.Vertices.OrderBy(() => {  }, v, new Vector3DComparer()).ToArray();
        ...
        public GetHashCode(poly: Polygon): number {
    ...
    export class FunPolygons {
        ...
        public static Heart(): Polygon {
            let newPoly: Polygon = new Polygon();
            let size: number = 0.12;
            let angle: number = ((3
                        * (Math.PI / 2))
                        * -1);
            let p1: Vector3D = new Vector3D(0, ((1.5 * size)
                            * -1));
            let p2: Vector3D = new Vector3D((size * -1), 0);
            let p3: Vector3D = new Vector3D(((size / 2)
                            * -1), size);
