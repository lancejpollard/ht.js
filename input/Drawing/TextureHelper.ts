...
module R3.Drawing {
    ...
    export class TextureHelper {
        ...
        public static SetLevels(levels: number): number {
        ...
        ///  </summary>
        public get ElementIndices(): List<number[]> {
        }
        public set ElementIndices(value: List<number[]>)  {
        }

        ///  <summary>
        ///  Sets up our list of element indices
        ///  </summary>
        public SetupElementIndices(poly: Polygon) {
            ...
            this.ElementIndices = new List<number[]>();
            for (let i: number = 0; (Math.Pow(2, i) <= m_maxSubdivisions); i++) {
                this.ElementIndices.Add(TextureHelper.TextureElements(numBaseTriangles, i));
            }

        }

        private static m_maxSubdivisions: number = 8;

        //  Must be a power of 2.
        /// ////////////////////////////////////////////////////////////// PLAYING AROUND WITH GEODESIC SADDLES
        private static CalcPointsUsingTwoSegments(seg1: Segment, seg2: Segment, divisions: number, g: Geometry): Vector3D[] {
            let points: List<Vector3D> = new List<Vector3D>();
            let s1: Vector3D[] = TextureHelper.SubdivideSegmentInGeometry(seg1.P1, seg1.P2, divisions, g);
            let s2: Vector3D[] = TextureHelper.SubdivideSegmentInGeometry(seg2.P2, seg2.P1, divisions, g);
            for (let i: number = 0; (i < divisions); i++) {
                points.AddRange(TextureHelper.SubdivideSegmentInGeometry(s1[i], s2[i], (divisions - i), g));
            }
        ...
        public static CalcViaProjections(p1: Vector3D, p2: Vector3D, p3: Vector3D, divisions: number, g: Geometry): Vector3D[] {
            ...
            let s1: Vector3D[] = seg1.Subdivide(divisions);
            let s2: Vector3D[] = seg2.Subdivide(divisions);
            for (let i: number = 0; (i < divisions); i++) {
        ...
        private static FindClosestPoint(v: Vector3D, list: Vector3D[]): Vector3D {
        ...
        ///  </summary>
        public static TextureCoords(poly: Polygon, g: Geometry, doGeodesicDome: boolean = false): Vector3D[] {
            ...
            if (((3 == poly.Segments.Count)
                        && doGeodesicSaddles)) {
                return TextureHelper.CalcViaProjections(poly.Segments[0].P1, poly.Segments[1].P1, poly.Segments[2].P1, divisions, g);
            }
            else {
                ...
                for (let s: Segment in poly.Segments) {
                    let s1: Vector3D[] = TextureHelper.SubdivideSegmentInGeometry(s.P1, poly.Center, divisions, g);
                    let s2: Vector3D[] = TextureHelper.SubdivideSegmentInGeometry(s.P2, poly.Center, divisions, g);
                    for (let i: number = 0; (i < divisions); i++) {
                        points.AddRange(TextureHelper.SubdivideSegmentInGeometry(s1[i], s2[i], (divisions - i), g));
                    }
        ...
        ///  </summary>
        public static SubdivideSegmentInGeometry(p1: Vector3D, p2: Vector3D, divisions: number, g: Geometry): Vector3D[] {
            ...
            let temp: Vector3D[] = TextureHelper.SubdivideRadialInGeometry(radial, divisions, g);
            let result: List<Vector3D> = new List<Vector3D>();
            for (let v: Vector3D in temp) {
        ...
        ///  </summary>
        private static SubdivideRadialInGeometry(radial: Segment, divisions: number, g: Geometry): Vector3D[] {
            let result: List<Vector3D> = new List<Vector3D>();
        ...
        ///  </summary>
        private static TriangularNumber(n: number): number {
            return (n
                        * ((n + 1)
                        / 2));
        }
        ...
        ///  </summary>
        public static TextureElements(numBaseTriangles: number, LOD: number): number[] {
            let divisions: number = m_maxSubdivisions;
            let stride: number = (divisions / (<number>(Math.Pow(2, LOD))));
            //  9 + 8 + 7 + 6 + 5 + 4 + 3 + 2 + 1
            let numVertsPerSegment: number = TextureHelper.TriangularNumber((divisions + 1));
            ...
            for (let count: number = 0; (count < numBaseTriangles); count++) {
                ...
                for (let i: number = 0; (i < divisions); i = (i + stride)) {
                    ...
                    for (let j: number = 0; (j
                                < (divisions - i)); j = (j + stride)) {
                        let idx1: number = (start1 + j);
                        let idx2: number = (start1
                                    + (j + stride));
        ...
        public static ElementGraph(numBaseTriangles: number, LOD: number): Record<number, number[]> {
            let result: Record<number, number[]> = new Record<number, number[]>();
            //  Brute force.
            let textureElements: number[] = TextureHelper.TextureElements(numBaseTriangles, LOD);
            let edgeToTriangles: Record<GraphEdge, List<number>> = new Record<GraphEdge, List<number>>();
            for (let i: number = 0; (i
                        < (textureElements.Length / 3)); i++) {
                let idx1: number = (i * 3);
                let idx2: number = ((i * 3)
                            + 1);
                let idx3: number = ((i * 3)
            ...
            let temp: Record<number, List<number>> = new Record<number, List<number>>();
