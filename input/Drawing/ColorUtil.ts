...
module R3.Core {
    ...
    export class ColorUtil {
        ...
        //  Copied from POV-Ray
        public static CH2RGB(H: number): Vector3D {
            let B: number = 0;
            let R: number = 0;
            let G: number = 0;
            if (((H >= 0)
        ...
        public static CHSL2RGB(hsl: Vector3D): Vector3D {
        ...
        public static AvgColor(colors: List<Color>): Color {
            if ((colors.Count == 0)) {
                return Color.FromArgb(0, 0, 0, 0);
            }

            let a: number = (<number>(colors.Select(() => {  }, (<number>(c.A))).Average()));
            let r: number = (<number>(colors.Select(() => {  }, (<number>(c.R))).Average()));
            let g: number = (<number>(colors.Select(() => {  }, (<number>(c.G))).Average()));
            let b: number = (<number>(colors.Select(() => {  }, (<number>(c.B))).Average()));
            return Color.FromArgb(a, r, g, b);
        }

        public static AvgColorSquare(colors: List<Color>): Color {
            if ((colors.Count == 0)) {
                return Color.FromArgb(0, 0, 0, 0);
            }
        ...
        public static InterpColor(c1: Color, c2: Color, input: number): Color {
        ...
        public static Inverse(c: Color): Color {
            return Color.FromArgb(255, (255 - c.R), (255 - c.G), (255 - c.B));
        }

        public static FromRGB(rgb: Vector3D): Color {
            if (rgb.DNE) {
                return Color.FromArgb(0, 255, 255, 255);
            }

            rgb = (rgb * 255);
            return Color.FromArgb(255, (<number>(rgb.X)), (<number>(rgb.Y)), (<number>(rgb.Z)));
        }

        public static AdjustH(c: Color, h: number): Color {
            let hsl: Vector3D = new Vector3D(c.GetHue(), c.GetSaturation(), c.GetBrightness());
            hsl.X = h;
            let rgb: Vector3D = ColorUtil.CHSL2RGB(hsl);
            return ColorUtil.FromRGB(rgb);
        }

        public static AdjustS(c: Color, s: number): Color {
            let hsl: Vector3D = new Vector3D(c.GetHue(), c.GetSaturation(), c.GetBrightness());
            hsl.Y = s;
            let rgb: Vector3D = ColorUtil.CHSL2RGB(hsl);
            return ColorUtil.FromRGB(rgb);
        ...
        public static AdjustL(c: Color, l: number): Color {
            if ((l > 1)) {
                l = 1;
            }
        ...
        ///  </summary>
        public static ColorAlongHexagon(incrementsUntilRepeat: number, increments: number): Color {
            ...
            let distAlongHex: number = ((<number>(increments)) * (6 / incrementsUntilRepeat));
