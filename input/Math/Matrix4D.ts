...
module R3.Math {
    ...
    export class Matrix4D {
        ...
        public constructor (rows: Vector3D[]) {
            this.Initialize();
            for (let i: number = 0; (i < 4); i++) {
                Data[i] = [
                        rows[i].X,
        ...
        ///  </summary>
        public get Item(i: number, j: number): number {
            return this.Data[i][j];
        }
        public set Item(value: number, i: number, j: number)  {
            this.Data[i][j] = value;
        }
        
        public get Item(i: number): Vector3D {
            return new Vector3D(this.Data[i]);
        }
        public set Item(value: Vector3D, i: number)  {
            this.Data[i] = [
                    value.X,
                    value.Y,
                    value.Z,
        ...
        public static Operator(m1: Matrix4D, m2: Matrix4D): Matrix4D {
            let result: Matrix4D = new Matrix4D();
            for (let i: number = 0; (i < 4); i++) {
                for (let j: number = 0; (j < 4); j++) {
        ...
        public static Operator(m: Matrix4D, s: number): Matrix4D {
            let result: Matrix4D = new Matrix4D();
            for (let i: number = 0; (i < 4); i++) {
                for (let j: number = 0; (j < 4); j++) {
                    result[i, j] = (m[i, j] * s);
        ...
        public static Transpose(m: Matrix4D): Matrix4D {
            let result: Matrix4D = new Matrix4D();
            for (let i: number = 0; (i < 4); i++) {
                for (let j: number = 0; (j < 4); j++) {
                    result[i, j] = m[j, i];
        ...
        ///  </summary>
        public static GramSchmidt(input: Matrix4D): Matrix4D {
        ...
        ///  Gram-Schmidt orthonormalize
        ///  </summary>
        public static GramSchmidt(input: Matrix4D, innerProduct: Func<Vector3D, Vector3D, number>, normalize: Func<Vector3D, Vector3D>): Matrix4D {
            let result: Matrix4D = input;
            for (let i: number = 0; (i < 4); i++) {
        ...
        ///  </summary>
        public RotateVector(input: Vector3D): Vector3D {
        ...
        ///  </summary>
        public static MatrixToRotateinCoordinatePlane(angle: number, c1: number, c2: number): Matrix4D {
