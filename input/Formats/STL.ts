...
module R3.Core {
    ...
    export class STL {
        ...
        public static SaveMeshToSTL(mesh: Mesh, fileName: String) {
            let sw: StreamWriter = File.CreateText(fileName);
            STL.WriteInternal(mesh, sw);
        }
        
        public static AppendMeshToSTL(mesh: Mesh, fileName: String) {
            let sw: StreamWriter = File.AppendText(fileName);
            STL.WriteInternal(mesh, sw);
        }
        
        public static AppendMeshToSTL(mesh: Mesh, sw: StreamWriter) {
            STL.WriteInternal(mesh, sw);
        }
        
        private static WriteInternal(mesh: Mesh, sw: StreamWriter) {
            sw.WriteLine("solid");
        ...
        private static WriteTriangle(sw: StreamWriter, tri: Mesh.Triangle) {
