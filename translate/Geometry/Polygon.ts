...
export class Segment implements ITransformable {
  ...
  ///  </summary>
  Subdivide(numSegments: number): Vector3D[] {
...
export class Polygon extends ITransformable {
  ...
  ///  </summary>
  static FromPoints(points: Vector3D[]): Polygon {
  ...
  ///  </summary>
  CreateEuclidean(points: Vector3D[]) {
    this.Segments.Clear()
    for (let i: number = 0; i < points.Length; i++) {
      let idx1: number = i
      let idx2: number = i + 1
  ...
  ///  </summary>
  GetIntersectionPoints(line: Circle): Vector3D[] {
...
export class PolygonEqualityComparer extends IEqualityComparer<Polygon> {
  Equals(poly1: Polygon, poly2: Polygon): boolean {
    let orderedVerts1: Vector3D[] = poly1.Vertices.OrderBy(
      () => {},
      v,
      new Vector3DComparer(),
    ).ToArray()
    let orderedVerts2: Vector3D[] = poly2.Vertices.OrderBy(
      () => {},
      v,
      new Vector3DComparer(),
    ).ToArray()
