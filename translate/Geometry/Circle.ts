...
export class Circle implements ITransformable {
  ...
  /* virtual */ Transform(m: Mobius) {
    this.TransformInternal(m)
  }

  /* virtual */ Transform(i: Isometry) {
  ...
  GetIntersectionPoints(segment: Segment): Vector3D[] {
    let p2: Vector3D
    let p1: Vector3D
    let result: number
    //  Are we a line?
...
export class CircleNE extends Circle implements ITransformable {
  ...
  get CenterNE(): Vector3D {}
  set CenterNE(value: Vector3D) {}

  Clone(): CircleNE {
    return MemberwiseClone()
  }

  /* override */ Reflect(c: Circle) {
    super.Reflect(c)
    this.CenterNE = c.ReflectPoint(this.CenterNE)
  }

  /* override */ Reflect(s: Segment) {
    super.Reflect(s)
    this.CenterNE = s.ReflectPoint(this.CenterNE)
  }

  /* override */ Transform(m: Mobius) {
    super.Transform(m)
    this.CenterNE = m.Apply(this.CenterNE)
  }

  /* override */ Transform(i: Isometry) {
    super.Transform(i)
