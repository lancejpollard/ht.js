...
export class PovRay {
  ...
  ///  </summary>
  static AppendFacets(facets: Sphere[], fileName: string) {
    let sw: StreamWriter = File.AppendText(fileName)
  ...
  ///  </summary>
  static AppendFacets(cells: H3.Cell[], fileName: string) {
  ...
  static #GetSpheres(
    ...
    let transformed: Vector3D[] = verts
      .Select(v => {
  ...
  static #Domain(
    ...
    let subtractSpheres: Sphere[] = PovRay.GetSpheres(
      facets,
      verts,
      interiorPoint,
      hDist / 2,
  ...
  static #SimplexFacets(
    ...
    for (let idx: number in include) {
      ...
      let others: Sphere[] = facets.Except([facet]).ToArray()
  ...
  static AppendEuclideanPolygons(polys: Polygon[], fileName: string) {
    let sw: StreamWriter = File.AppendText(fileName)
