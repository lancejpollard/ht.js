...
export class Shapeways {
  ...
  ///  </summary>
  AddCurve(points: Vector3D[], size: number) {
    let sizeFunc: System.Func<Vector3D, number>
    size
  ...
  AddClosedCurve(points: Vector3D[], radii: number[]) {
    if (points.Length < 3) {
      throw new Error('Argument Error')(
  ...
  AddCurve(points: Vector3D[], radii: number[]) {
    if (points.Length < 2) {
      throw new Error('Argument Error')(
        'AddCurve requires at least two input points.',
      )
  ...
  AddArc(
    ...
    let points: Vector3D[] = Shapeways.CalcArcPoints(
      center,
      arcRadius,
      v1,
      normal,
  ...
  AddCornucopia(
    ...
    let disksOuter: List<Vector3D[]> = this.CalcDisks(
      outerPoints,
      outerSizeFunc,
    )
    let disksInner: List<Vector3D[]> = this.CalcDisks(
      innerPoints,
      innerSizeFunc,
      /* reverse:*/ true,
    )
  ...
  ///  Adds an annulus.
  ///  </summary>
  #AddRing(disk1Points: Vector3D[], disk2Points: Vector3D[]) {
  ...
  ///  </summary>
  AddSegment(d1: Vector3D[], d2: Vector3D[]) {
