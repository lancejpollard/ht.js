...
export class ImageSpace {
  ...
  Width(width: number): number {
    let percent: number = width / (this.XMax - this.XMin)
  ...
  Height(height: number): number {
    let percent: number = height / (this.YMax - this.YMin)
  ...
  Pixel(point: Vector3D): Vector3D {
    let xPercent: number =
      (point.X - this.XMin) / (this.XMax - this.XMin)
    let yPercent: number =
      (point.Y - this.YMin) / (this.YMax - this.YMin)
    let x: number = xPercent * this.m_width
    let y: number = this.m_height - yPercent * this.m_height
  ...
  Point(Pixel: Vector3D): Vector3D {
    return new Vector3D(
      this.XMin + (Pixel.X / this.m_width) * (this.XMax - this.XMin),
      this.YMax - (Pixel.Y / this.m_height) * (this.YMax - this.YMin),
      0,
    )
  }
  ...
  Pixel(point: VectorND): VectorND {
    let result: Vector3D = this.Pixel(
      new Vector3D(point.X[0], point.X[1], point.X[2]),
    )
