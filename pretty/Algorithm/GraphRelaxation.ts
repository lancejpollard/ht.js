export class GraphNode {
  Position: Vector3D
  constructor(pos: VectorND, vel: VectorND) {
    Position = pos
    Velocity = vel
  ...
  get Position(): VectorND {}
  set Position(value: VectorND) {}

  get Velocity(): VectorND {}
  set Velocity(value: VectorND) {}
...
export class Graph {
  ...
  set Nodes(value: List<GraphNode>) {}

  get Edges(): List<GraphEdge> {}
  set Edges(value: List<GraphEdge>) {}
  ...
  ///  </summary>
  SetupCompleteGraph(n: number) {
    this.Nodes.Clear()
    this.Edges.Clear()
    let southPole: GraphNode = new GraphNode(
      new VectorND([0, 0, 0, -1]),
      new VectorND(4),
    )
    southPole.FullLock()
  ...
  AddEdge(e: GraphEdge) {
    ...
    if (m_connections.TryGetValue(e.V1, /* out */ vals)) {
      ...
      m_connections[e.V1] = new List<number>([e.V2])
    }

    this.Edges.Add(e)
  }

  m_connections: Record<number, List<number>>
  ...
  Connected(n1: number, n2: number): boolean {
    ...
    let vals: List<number>
    if (
      this.m_connections.TryGetValue(n1, /* out */ vals) &&
      vals.Contains(n2)
    ) {
      return true
    }
    ...
    if (
      this.m_connections.TryGetValue(n2, /* out */ vals) &&
      vals.Contains(n1)
    ) {
      return true
    }
...
export class GraphRelaxation {
  ...
  ///  </summary>
  get EdgeAttraction(): number {}
  set EdgeAttraction(value: number) {}

  ///  <summary>
  ///  A repulsion force between all nodes.
  ///  Nodes act like similarly charged ions.
  ///  </summary>
  get NodeRepulsion(): number {}
  set NodeRepulsion(value: number) {}

  ///  <summary>
  ///  A repulsion force between all edges.
  ///  </summary>
  get EdgeRepulsion(): number {}
  set EdgeRepulsion(value: number) {}
  ...
  #CalcAccelerations(): VectorND[] {
    ...
    for (let kvp in this.Graph.m_connections) {
      ...
      for (let j: number in kvp.Value) {
        let edgeForce: VectorND = this.CalculateForce(
          this.Graph.Nodes[i],
          this.Graph.Nodes[j],
          this.EdgeAttraction,
          /* square:*/ false,
    ...
    if (edgeRepulse) {
      ...
      for (let i: number = 0; i < count; i++) {
        for (let j: number = i + 1; j < count; j++) {
          ...
          let center1: GraphNode = new GraphNode(
            (this.Graph.Nodes[n1].Position +
              this.Graph.Nodes[n2].Position) /
              2,
            new VectorND(this.m_dim),
          )
          let center2: GraphNode = new GraphNode(
            (this.Graph.Nodes[n3].Position +
              this.Graph.Nodes[n4].Position) /
              2,
            new VectorND(this.m_dim),
          )
          let force: VectorND =
  ...
  #UpdatePositionAndVelocity(node: GraphNode, acceleration: VectorND) {
    // if( allLocked )
    //     return;
  ...
  #ZeroComponents(components: VectorND, /* ref */ input: VectorND) {
