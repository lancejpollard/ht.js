  ...
  /* override */ EditValue(
    ...
    if (context != null && provider != null) {
      ...
      if (this.editorService != null) {
        ...
        trackBar.ValueChanged.addEventListener(
          new EventHandler(this.ValueChanged),
        )
        trackBar.MouseLeave.addEventListener(
          new EventHandler(this.MouseLeave),
        )
        trackBar.Minimum = 0
        ...
        let ra: RangeAttribute = <RangeAttribute>(
          prop.Attributes[typeof RangeAttribute]
        )
        this.valueLow = ra.Low
        this.valueHigh = ra.High
        //  Set the corresponding trackbar value.
        let percent: number =
          (System.Convert.ToDouble(value) - this.valueLow) /
